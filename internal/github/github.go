// Package github fala com a API REST do GitHub e implementa o OAuth Device
// Flow (login por código, sem PAT manual). Porte fiel do github.go do /stash,
// desacoplado do Wails:
//
//   - *Config            → *config.Config (mesmo settings.json da suíte Adila;
//     o token logado no /ide vale aqui)
//   - emit(...)           → hook Emit injetável (no-op por padrão); a camada
//     Wails (Fase 7) liga em app.Event.Emit
//   - ctx do Wails        → Start(ctx); default context.Background()
//   - pickDirectory()     → removido (era Wails-only, como o PickRepoFolder do
//     motor git): o frontend informa a pasta de clone
//
// O resto da superfície (Device Flow, repos, PRs, reviews, comentários, clone
// com progresso) é idêntico ao stash para permitir re-sync com o upstream.
package github
...
// Service é o cliente GitHub. Emit é o gancho de eventos (no-op se nil);
// a camada Wails injeta app.Event.Emit na Fase 7.
type Service struct {
	ctx    context.Context
	cfg    *config.Config
	client *http.Client
...
// Start injeta o context de vida do app (Wails startup). Opcional: sem chamar,
// usa context.Background().
func (g *Service) Start(ctx context.Context) {
	if ctx != nil {
		g.ctx = ctx
	}
}

func (g *Service) emit(name string, data ...any) {
	if g.Emit != nil {
		g.Emit(name, data...)
	}
}
...
func (g *Service) IsAuthenticated() bool {
	return g.token() != ""
}
...
func (g *Service) StartDeviceFlow() (DeviceFlowStart, error) {
	form := url.Values{}
	...
	// Best-effort: a UI também mostra a URL + código como fallback.
	_ = browser.OpenURL(raw.VerificationURI)
...
func (g *Service) PollDeviceToken(deviceCode string, interval int) error {
	if deviceCode == "" {
...
func (g *Service) CancelDeviceFlow() {
	g.cancelPolling()
}

func (g *Service) exchangeDeviceCode(deviceCode string) (token string, slowDown, pending bool, err error) {
	form := url.Values{}
	form.Set("client_id", githubClientID)
...
func (g *Service) apiRequest(method, path string, body any) (*http.Response, error) {
	tok := g.token()
	if tok == "" {
...
func (g *Service) GetUser() (GitHubUser, error) {
...
func (g *Service) ListMyRepos(limit int) ([]GitHubUserRepo, error) {
...
func (g *Service) CreatePullRequest(owner, repo, base, head, title, body string) (*PullRequestInfo, error) {
...
func (g *Service) ListPullRequests(owner, repo, state string) ([]PullRequestSummary, error) {
	if owner == "" || repo == "" {
...
func (g *Service) GetPullRequest(owner, repo string, number int) (*PullRequestDetail, error) {
...
// Vazio assume "merge".
func (g *Service) MergePullRequest(owner, repo string, number int, method string) error {
	if owner == "" || repo == "" || number <= 0 {
...
func (g *Service) ListIssueComments(owner, repo string, number int) ([]IssueComment, error) {
	if owner == "" || repo == "" || number <= 0 {
...
func (g *Service) CreateIssueComment(owner, repo string, number int, body string) (*IssueComment, error) {
...
func (g *Service) ListReviews(owner, repo string, number int) ([]PullRequestReview, error) {
	if owner == "" || repo == "" || number <= 0 {
		return nil, errors.New("parâmetros incompletos")
	}
	resp, err := g.apiRequest("GET",
...
func (g *Service) ListReviewComments(owner, repo string, number int) ([]ReviewComment, error) {
	if owner == "" || repo == "" || number <= 0 {
		return nil, errors.New("parâmetros incompletos")
	}
	resp, err := g.apiRequest("GET",
...
func (g *Service) ListPullRequestCommits(owner, repo string, number int) ([]PullRequestCommit, error) {
	if owner == "" || repo == "" || number <= 0 {
		return nil, errors.New("parâmetros incompletos")
	}
	resp, err := g.apiRequest("GET",
...
func (g *Service) ListPullRequestFiles(owner, repo string, number int) ([]PullRequestFile, error) {
	if owner == "" || repo == "" || number <= 0 {
		return nil, errors.New("parâmetros incompletos")
	}
	resp, err := g.apiRequest("GET",
...
func (g *Service) CreateReview(owner, repo string, number int, event, body string, comments []ReviewCommentInput) (*PullRequestReview, error) {
	if owner == "" || repo == "" || number <= 0 {
...
func (g *Service) CreateReviewComment(owner, repo string, number int, commitID, path string, line int, side, body string) (*ReviewComment, error) {
...
func (g *Service) ReplyToReviewComment(owner, repo string, number int, inReplyTo int64, body string) (*ReviewComment, error) {
...
func (g *Service) CloneRepo(cloneURL, parentDir, name string) (string, error) {
	...
	g.emit("github:clone-progress", CloneProgress{CloneURL: cloneURL, Phase: "Iniciando", Percent: 0})
	...
	for scanner.Scan() {
		...
		if m := gitProgressLine.FindStringSubmatch(line); m != nil {
			...
			g.emit("github:clone-progress", CloneProgress{
				CloneURL: cloneURL,
				Phase:    phase,
				Percent:  percent,
			})
...
// AuthenticatedURL injeta o token atual numa URL https do GitHub para o
// `origin` poder dar push/pull em repo privado sem credential helper. Mesmo
// tradeoff do CloneRepo: o token vai parar no .git/config local (não é
// commitado). URLs ssh ou sem token passam intactas.
func (g *Service) AuthenticatedURL(raw string) string {
	return injectTokenIntoURL(strings.TrimSpace(raw), g.token())
}
